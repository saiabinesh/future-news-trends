...
def inferTopics(test_corpus,split_offset):
    ...
    test_dictionary = corpora.Dictionary.load("dictionary_"+str(i)+"_"+str(notopics)+"topics.dict")
    #loading the previously saved topic model.These are models that used to make topic inferences about documents
    lda = models.LdaMulticore.load("model_"+str(i)+"_"+str(notopics)+"topics.lda")
...
conn = sqlite3.connect('topic_frequencies_500_docs.db')
c = conn.cursor()
